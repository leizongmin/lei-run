...
// 异步执行命令
global.aexec = function (cmd, execOpts) {
  process.send({ cmd, execOpts });
};
...
function showTasks() {
  ...
  if (names.length > 0) {
    ...
    for (const name of names) {
      console.log(tab + color.blue(name));
      const fn = tasks[name];
      if (fn.description) {
        console.log(color.green(tab + ' -- ' + fn.description));
      }
    }
    emptyLine();
    console.log('type the following command to run a task:');
    emptyLine();
    console.log(tab + color.magenta.bold('run ' + color.blue('<task>')));
    emptyLine();
    console.log('for example:');
    emptyLine();
    console.log(tab + color.magenta.bold('run ' + color.blue(names[0])));
    longLine();
  } else {
